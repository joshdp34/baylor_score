...
+ It assumes familiarity with R/RStudio.
...
Tennis court surfaces differ.  "Hard courts" are the most common in modern tennis, but clay and grass courts still exist.  In fact, two of the four major tournaments are not played on hard courts: Wimbledon (grass) and the French Open (clay).  A "clay court season" precedes the French Open and similarly there is a "grass court season" leading up to Wimbledon.

The surface impacts the game in terms of strategy, training, and how play progresses. For example, a Koka sports author suggestion that there is “a grind on hard courts that does not show up on TV. The surface is tough on legs and backs" has implications for training.  

```{r}
#| echo: false
#| fig-cap: "https://blog.kokasports.com/tennis-court-surfaces-impact/"
#| out-width: "70%"
knitr::include_graphics("Picture1.png")
```

One tennis academy gives tips and tricks for play on different surfaces and notes: "clay courts often witness extended rallies due to the slower ball speed upon impact with the surface."  Such an observation has ramifications for training and preparation for a clay court event.  They further discuss the importance of agility on clay courts, and speed and athleticism on grass.


```{r}
#| echo: false
#| fig-cap: "https://motennistraining.com/the-impact-of-surface-type-on-tennis-strategy-and-play/"
#| out-width: "70%"
knitr::include_graphics("Picture2.png")
```

To fully address the differences on play due to the surface requires carefully designed studies. In this module, you will compute the required sample size for studies of research questions relating to the differences due to surface that could impact how players train and coaches strategize.  

A "pilot" data set is used to estimate parameters needed for the calculations using data from Wimbledon 2023 matches, which is a grass court tournament.  Sample size calculations alwasy require estimates of unkwown parameters and such limited data is all that is available. 
...
::: column-margin :::
**A note on the experiment**: careful design is required for the experiment itself; we do not discuss considerations in this module.  In the examples to follow we assume the design is such that the indepencence assumptions of the statistical tests are valid.  Can you think of how best to collect data for these studies to ensure this assumption holds?  The subject "design of experiments (DOE)" is focused on the issues with how best to plan studies.
:::
...
$$D = \frac{\mu_2 - \mu_1}{sd}$$
where sd is the estimated standard deviation.  If the average distance on clay increases by one standard deviation, then $D = 1$.  In other words, Cohen's D is the increase in terms of the standard deviation.  If $D = 0.5$ that would be an increase of half of a standard deviation,
...
a.  What value of delta leads to a small effect size of D = 0.25 standard deviation increase?

b.  What value of delta produces a large effect size of D = 1 sd increase?
...
The result is a "medium" effect size, but practically that seems like an unlikely change.  Even though clay might reduce the serve advantage, it still probably exists.  Let's consider reducing the advantage to 65\%:
...
a.  Modify the sample size calculation so that the difference in proportions is still 0.1 but based on 70\% for p1 and 60\% for p2.  What is the resulting sample size?  How does this compare to the sample size in the example using 75\% and 65\%?  
...
In this module we computed sample sizes for studies to answer questions that tennis players and coaches might find interesting in preparing for matches on different tennis court surfaces.  The first was the distance players run on clay compared to grass, which has implications for training.  The second was the importance of the first serve on these surfaces which would impact strategy as well as practice regimens and player development. 

The module illustrates the challenge of sample size computations to find reasonable parameter estimates.  Further, you learned how changing assumptions about these inputs impacts the sample size estimates.  Given such uncertainty, it is important to consider various scenarios and choose a sample size for the study that is most likely to produce adequate power for the most likely.  Finally, it is clear that sample size calculation does not ensure meaningful study results but hopefully improves those chances.
