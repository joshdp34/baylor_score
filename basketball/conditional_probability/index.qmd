...
**Intended student level:** Introductory statistics (first-year college students, including students who are not statistics or data science majors).

**Estimated completion time:** 30-45 minutes.

::: {.callout-note collapse="false" title="Facilitation notes" appearance="minimal"}

+ Introductory statistics (first-year college students, including students who are not statistics or data science majors).

+ It assumes a basic familiarity with `R`.

+ Students should be provided with the following data file (.csv)
    + [pgfreethrow.csv](https://github.com/kgfitzgerald/baylor_apu_score/blob/main/basketball/conditional_probability/pgfreethrow.csv)
...
A **free throw** is a basketball shot taken from behind the free throw line without a defender contesting the shot. Each successful free throw is worth one point ([NBA rules](https://official.nba.com/rule-no-4-definitions/)). Free throws are awarded after certain fouls or rule violations. A **trip to the free throw line** refers to the set of free throw attempts a player is awarded at one time.
...
-   Estimate the probability that two events both occur using data
...
Why might the first shot tell us something about the second? After a miss, a player might adjust their shooting motion or feel more pressure; fatigue or game conditions might also affect both attempts. These are possible explanations to explore, rather than conclusions we can draw from the data alone. If the two shot outcomes are *independent*, knowing whether the first shot was made or missed does not change the probability of making the second. Estimating the second-shot probability separately after a make and after a miss lets us explore that assumption. Differences in the observed proportions can also arise from chance, so a difference alone does not establish dependence or show that the first shot causes the second shot's outcome.
...
For Paul George's free throw attempts, let's look at the following conditional probabilities:
...
$$
Here, $P(A\text{ and }B)$ is the probability that both events occur. The event "A and B" is also called the **intersection** of $A$ and $B$, written $A \cap B$. For example, making the first shot and making the second shot in the same trip is an intersection of two events.
...
Now we can divide this value by the total number of first shots. In the `free_throw_data` dataframe, this can be determined by just counting the number of times `shot==1`. Dividing the count stored in `first_made` by the total number of rows where `shot==1` gives us the estimated probability for making the first shot.
...
To estimate the probability $P(B_2\text{ and }B_1)$, we need to filter the data where the first shot is made and the second shot is made in the same trip to the free throw line. Since the data frame is in chronological order, we can assume the previous row in the data frame corresponds to the previous free throw attempt. In `R`, we can use the `lag` function to refer to the previous row value in our dataframe. The code `lag(make)==TRUE` will find the rows that come right after a row where `make==TRUE`. Combined with `shot==2` and `make==TRUE`, this identifies trips where both the first and second shots were made.
...
-   How to estimate the probability that two events both occur using data
