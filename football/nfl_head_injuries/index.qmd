...
theme_update(
  ...
  plot.title = element_text(family = "Times New Roman", face = "bold", color = "#17476b", size = 14),
  axis.title = element_text(family = "Times New Roman", face = "bold", color = "#17476b"),
  axis.text = element_text(family = "Times New Roman", color = "#333333")
)
...
inclusion_counts |> 
  gt() |>
  tab_header(
    title = "Sample Inclusion Process"
  ) |>
  cols_label(
    Stage = "Inclusion Stage",
    Unit = "Unit",
    Count = "Count"
  ) |>
  fmt_number(
  ...
  tab_options(
    ...
    table_body.hlines.color = "#d9e2f3"
  )
```
...
age_summary |>
  gt() |>
  tab_header(
    title = "Age Summary by Position Group"
  ) |>
  cols_label(
  ...
  fmt_number(
    columns = c(mean_age, median_age, sd_age, min_age, max_age),
    decimals = 2
  ) |>
  tab_style(
    style = cell_text(weight = "bold"),
  ...
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_body(columns = position_group)
  ) |>
  tab_options(
    table.font.names = "Times New Roman",
    table.font.size = px(12),
...
::: {.callout-note collapse="true" title="Run an ANOVA in R Studio"}
```{r}
...
anova_terms |>
  gt() |>
  tab_header(
    title = md("**Analysis of Variance (ANOVA) Results**"),
  ...
  tab_style(
    style = cell_text(color = "#e2ecf5"),
    locations = list(cells_title(groups = "subtitle"))
  ) |>
  tab_options(
    table.font.names = "Times New Roman",
    table.font.size = px(16), # Equivalent to 12pt
    heading.background.color = "#17476b",
    column_labels.background.color = "#f4f7f9",
    column_labels.font.weight = "bold",
    column_labels.border.bottom.color = "#17476b",
    column_labels.border.bottom.width = px(2),
    table_body.hlines.color = "#ebebeb",
    table.border.bottom.color = "#17476b",
...
::: {.callout-note collapse="true" title="Run Tukey's HSD in R Studio"}
...
::: {.callout-note collapse="true" title="Run Welch's ANOVA in R Studio"}
...
# 1. Run the test and convert the raw console output into a clean dataframe
welch_2012 <- oneway.test(
  age ~ position_group, data = offense_2012, var.equal = FALSE
)
welch_df <- tidy(welch_2012)
...
welch_df |>
  ...
  tab_header(
    title = md("**Welch's One-Way ANOVA**"),
    subtitle = "2012/2013 Season)"
  ) |>
  # Safely apply white text to the titles on the blue banner
  tab_style(
    style = cell_text(color = "white"),
    locations = list(cells_title(groups = "title"))
  ) |>
  ...
  # Enforce Times New Roman 12pt and blue background rules
  tab_options(
    table.font.names = "Times New Roman",
    table.font.size = px(16), # Equivalent to 12pt
    
    # Deep blue main header banner
    heading.background.color = "#17476b",
    
    # Light gray background for column headers with a blue bottom border
    column_labels.background.color = "#f4f7f9",
    column_labels.font.weight = "bold",
    column_labels.border.bottom.color = "#17476b",
    column_labels.border.bottom.width = px(2),
    
    # Clean internal gridlines
    table_body.hlines.color = "#ebebeb",
    table.border.bottom.color = "#17476b",
    table.border.bottom.width = px(2)
  ) |>
  # Rename columns to clear statistical labels
  cols_label(
    num.df = "Numerator df",
  ...
  # Round numbers cleanly (p-values get 4 decimals, statistics get 2 or 3)
  fmt_number(
...
::: {.callout-note collapse="true" title="Run Krusal-Wallis Test in R Studio"}
...
# 1. Run the test and convert the raw console output into a clean dataframe
kruskal_2012 <- kruskal.test(age ~ position_group, data = offense_2012)
kruskal_df <- tidy(kruskal_2012)

# 2. Format the dataframe into your styled theme table
kruskal_df |>
  gt() |>
  cols_hide(columns = method) |> 
  # Add table titles matching your project layout
  tab_header(
    title = md("**Kruskal-Wallis Rank Sum Test**"),
    subtitle = "2012/2013 Season)"
  ) |>
  tab_style(
    style = cell_text(color = "white"),
...
tibble(
  ...
  gt() |>
  tab_header(
    title = "Comparison of Statistical Methods"
  ) |>
  cols_label(
    Method = "Method",
    Target = "Statistical Target",
    p_value = "p-value",
    `Decision at 0.05` = "Decision (α = 0.05)"
  ) |>
  fmt_number(
    columns = p_value,
    decimals = 4
...
`r fmt(min(wilcoxon_2012$p.value, na.rm=TRUE), 4)`. No pair is significant at 0.05 under
this procedure. An omnibus test and adjusted pairwise tests can reach different
decisions: they ask different questions and use different rejection rules.
An omnibus rejection does not guarantee that this particular pairwise procedure
...
6. **Follow up.** For teaching comparison, compute Tukey intervals and
   Holm-adjusted pairwise Wilcoxon p-values even if an omnibus test is not
...
:::{.callout-note collapse=true title="Load, Summarize, and Analyze Data in R Studio"}
```{r}
...
6. Why might an omnibus test reject while adjusted pairwise tests do not?
...
p-values exceed 0.05. The omnibus and pairwise methods have different hypotheses
...
welch_2013 <- oneway.test(
  age ~ position_group,
  data = offense_2013
)

kruskal_2013 <- kruskal.test(
  age ~ position_group,
  data = offense_2013
)
...
tibble(
  ...
  Statistic = c(
    tidy(anova_2013)$statistic[1],
    unname(welch_2013$statistic),
    unname(kruskal_2013$statistic)
  ),
  p_value = c(
    tidy(anova_2013)$p.value[1],
    welch_2013$p.value,
  ...
  gt() |>
  tab_header(
    title = "Comparison of Statistical Tests"
  ) |>
  cols_label(
    Method = "Method",
...
wilcoxon_2013$p.value |>
  ...
  gt() |>
  tab_header(
    title = "Pairwise Wilcoxon Comparisons"
  ) |>
  cols_label(
    Group = "Position Group",
    `Compared with` = "Compared With",
    p_value = "Holm-Adjusted p-value"
  ) |>
  fmt_number(
    columns = p_value,
    decimals = 4
